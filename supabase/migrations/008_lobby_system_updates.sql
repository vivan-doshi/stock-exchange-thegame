...
ALTER TABLE games
DROP CONSTRAINT IF EXISTS games_current_round_check;
...
ALTER TABLE games
DROP CONSTRAINT IF EXISTS games_current_transaction_check;
...
-- Make max_players required going forward
ALTER TABLE games
...
-- Update constraint to allow any player count within variant limits (not enforced at row level)
ALTER TABLE games
DROP CONSTRAINT IF EXISTS player_count_variant_check;
